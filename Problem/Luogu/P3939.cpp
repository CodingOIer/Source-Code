...
int main()
{
    ...
    for (int i = 1; i <= m; i++)
    {
        ...
        if (op == 1)
        {
            int l, r, c;
            io >> l >> r >> c;
            io << int(std::upper_bound(cl[c].begin(), cl[c].end(), r) - std::lower_bound(cl[c].begin(), cl[c].end(), l))
               << '\n';
        }
        else
        {
            int x;
            io >> x;
            void(p[x] != p[x + 1] && (++(*std::lower_bound(cl[p[x]].begin(), cl[p[x]].end(), x)),
                                      --(*std::lower_bound(cl[p[x + 1]].begin(), cl[p[x + 1]].end(), x + 1)),
                                      std::swap(p[x], p[x + 1]), 1));
        }
    }
    return 0;
}